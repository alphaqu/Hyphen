...
import dev.quantumfusion.hyphen.scan.StructParameters;
 ...
 * If a {@link TypeStruct} gets defined by {@link Struct#resolve(Struct)} all of the {@link ParameterStruct} Structs coming from the source will also be defined.
 */
public class ClassStruct extends Struct {
	...
	public void resolve(Struct value) {
		if (value instanceof ClassStruct classStruct) {
			try {
				...
				for (int i = 0; i < this.parameters.list.size(); i++) {
					this.parameters.getParameter(i).resolve(classStruct.parameters.getParameter(i));
				}
			} catch (Throwable throwable) {
				throw new HyphenException("Struct " + value.simpleString() + " is incompatible with " + this.simpleString(), throwable, null);
			}

			return;
		}
		throw new IllegalArgumentException("Incompatible classes, this " + this + " is not compatible with " + value);
	}
	...
	public String toString() {
		...
		return this.aClass.getName() + super.toString() + parameters;
	}
